...
#ggsave(file.path(figureOut, paste(Sys.Date(), "Figure_metro_mic.png", sep="_")))
```
...
MICtable <- micDF1 %>%
  ...
  mutate(`Metronidazole resistance`=case_when(minMIC>=32 & maxMIC>=32 & CT==FALSE ~ "\\+",
         ...
         `Colonies`=case_when(CT==TRUE~"*",
                              CT==FALSE~"")) %>%
  select(Species, Strain, `MIC range`, `Metronidazole resistance`) %>%
  kbl(escape=FALSE, caption="Table 2. Minimum inhibitory concentration and resistance of metronidazole", align="lllc") %>%
  kable_classic(full_width=TRUE, html_font = "Arial") %>%
  #kable_styling(protect_latex = TRUE) %>%
  footnote(general="+ Resistant; - Susceptible; * Variable susceptibility; ** Heteroresistant (zone of inhibtion with colonies throughout)", general_title = "")
